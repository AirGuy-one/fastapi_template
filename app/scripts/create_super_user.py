...
class SuperUserGenerator:
    ...
    def _get_user_input(prompt, valid_options=None):
        while True:
            ...
            if valid_options and user_input not in [option.value for option in valid_options]:
                logger.info(f"Некорректный ввод: {', '.join(option.value for option in valid_options)}.")
                continue
            return user_input
    ...
    def save_to_database(self):
        admin_user = User(login=self.login, password=self.password, is_active=True, role=UserRole.super_user)
